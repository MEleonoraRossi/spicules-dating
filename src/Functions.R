                       ...
                       out_dat, dataset, time_unit = 100 )
{
  ...
  # 4. Get matching nodes with calib names -- for this project, this is not a 
  #    list, and thus the next line has changed
  ...
  
{
  ...
  if( ! dir.exists( paste( main_wd, "plots/effVSuser/", sep = "" ) ) ){
    dir.create( paste( main_wd, "plots/effVSuser/", sep = "" ) )
  }
  for( dat_i in dat ){
    if( ! dir.exists( paste( main_wd, "plots/effVSuser/", dat_i,  sep = "" ) ) ){
    dir.create( paste( main_wd, "plots/effVSuser/", dat_i, sep = "" ) )
    }
    if( ! dir.exists( paste( main_wd, "plots/effVSuser/", dat_i, "/", out, sep = "" ) ) ){
      dir.create( paste( main_wd, "plots/effVSuser/", dat_i, "/", out,  sep = "" ) )
    }

    # In case users have not passed a string of length 3, just get the first three
    # letters
    png( filename = paste( main_wd, "plots/effVSuser/", dat_i, "/", out, "_",
                           dat_i, "_", clock,".jpg", sep = "" ),
    ...
    for( i in 1:length( calibs[,2] ) ){
      ...
      if( length( is_L ) == 1 ){
        ...
        tmp_x   <- seq( tmp_min-1, tmp_min+5, 0.001 )
      ...
      if ( ind == TRUE ){
        if( ! dir.exists( paste( main_wd, "plots/effVSuser/", dat_i, "/", out, "/ind", sep = "" ) ) ){
          dir.create( paste( main_wd, "plots/effVSuser/", dat_i, "/", out, "/ind", sep = "" ) )
        }
        if( ! dir.exists( paste( main_wd, "plots/effVSuser/", dat_i, "/", out, "/ind/", sep = "" ) ) ){
          dir.create( paste( main_wd, "plots/effVSuser/", dat_i, "/", out, "/ind/", sep = "" ) )
        }
        if( ! dir.exists( paste( main_wd, "plots/effVSuser/", dat_i, "/", out, "/ind/", clock, sep = "" ) ) ){
          dir.create( paste( main_wd, "plots/effVSuser/", dat_i, "/", out, "/ind/", clock, sep = "" ) )
        }
        png( filename = paste( main_wd, "plots/effVSuser/",  dat_i, "/", out, "/ind/", clock, "/CheckUSPvsEP_tn", 
    ...
    lines( x = 1, y = 1, col = "white", xaxt = "n", yaxt = "n", xlab = '', ylab = '' )
