...
## POSTERIOR ##
path_NoSpi <- paste( home_dir, "../01_PAML/01_MCMCtree/analyses/posterior/NoSpi/IR/",
                     sep = "" )
path_SpiPor <- paste( home_dir, "../01_PAML/01_MCMCtree/analyses/posterior/SpiPor/IR/",
                      sep = "" )
...
# Chain 1 ran under other parameters and chain 6 seems to have been exploring
# the wrong area of the parameter space. These two chains have been removed
# The next object has been updated for this analysis
num_chains <- 4
sum_NoSpi  <- sum_MCMC( num_dirs = c(2, 3, 4, 5), delcol = delcol_post,
                        data_dir = path_NoSpi,
...
pdf( paste( outchecks_dir,
            "ESS_and_chains_convergence/Convergence_plot_post_NoSpi_IR.pdf",
            sep = "" ),
     paper = "a4" )
plot_convergence( name_dir_dat = "Post - NoSpi+IR",
...
pdf( paste( outchecks_dir,
            "ESS_and_chains_convergence/Convergence_plot_post_NoSpi_IR_FILT.pdf",
            sep = ""), 
     paper = "a4" )
plot_convergence( name_dir_dat = "Post FILT - NoSpi+IR",
...
min( ESS_post$stats$Rhat ) # 0.9999337
max( ESS_post$stats$Rhat ) # 1.006146
...
post_half2  <- apply( X = sum_SpiPor$mean[(half_chains+1):num_chains,],
                      MARGIN = 2, FUN = mean )
pdf( paste( outchecks_dir,
            "ESS_and_chains_convergence/Convergence_plot_post_SpiPor_IR.pdf",
            sep = "" ),
     paper = "a4" )
plot_convergence( name_dir_dat = "Post - SpiPor+IR",
                  mean_divt1 = post_half1,
                  mean_divt2 = post_half2, num_runs = num_chains )
dev.off()
# Plot traces -- only uncomment and run if enough space in disk!
...
check_quantiles( dat = sum_SpiPor, threshold = th, main_chain = main_ch,
                 ...
                 outdir = path_SpiPor )
#>
#> END CHECK: There is 1 problematic chain: 3.

# 2. Get filtered summary object and generate convergence plots with filtered
# dataset
filt_chains <- c( 1:2, 4:6 )
write.table( x = t(c( 1:2, 4:6 )),
             file = paste( path_SpiPor, "chains_kept.txt", sep = "" ),
             quote = FALSE, row.names = FALSE, col.names = FALSE )
filt_sum <- sum_MCMC( num_dirs = filt_chains, delcol = delcol_post,
                      data_dir = path_SpiPor,
                      num_divt = num_divt, node_calib = calib_nodes_all$SpiPor,
                      dataset = "SpiPor_IR_FILT",
                      perc = perc, def_samples = def_samples,
                      prior = FALSE,
                      # In this case, I want the output dir to be
                      # created within the same data dir, but it
...
FILT_half1 <- apply( X = filt_sum$mean[1:3,], MARGIN = 2, FUN = mean )
FILT_half2 <- apply( X = filt_sum$mean[4:5,], MARGIN = 2, FUN = mean )
pdf( paste( outchecks_dir,
            "ESS_and_chains_convergence/Convergence_plot_post_SpiPor_IR_FILT.pdf",
            sep = ""), 
     paper = "a4" )
plot_convergence( name_dir_dat = "Post FILT - SpiPor+IR",
                  mean_divt1 = FILT_half1,
                  mean_divt2 = FILT_half2, num_runs = num_filt_chains )
dev.off()
# Update objects
...
## Max.    49641    46930
# Calculate Rhat, min and max. Good if max Rhat <= 1.05
min( ESS_post$stats$Rhat ) # 0.9999561
max( ESS_post$stats$Rhat ) # 1.005087
# Number of samples collected throughout
dim( sum_SpiPor$all_mcmc ) # [1] 100005     69
