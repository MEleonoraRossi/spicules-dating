...
delcol_prior   <- 5 # There is one column per mu (5) : mu1,mu2,mu3,mu4,mu5
...
# approaches.
num_dirs    <- 2
path_prior  <- c( paste( home_dir, "../01_PAML/01_MCMCtree/analyses/prior/mcmc_files_NoSpi", sep = "" ),
                  paste( home_dir, "../01_PAML/01_MCMCtree/analyses/prior/mcmc_files_SpiPor", sep = "" ) )
...
names( mcmc_priors ) <- c( "NoSpi", "SpiPor" )
count <- 0
...
for( i in 1:num_dirs ){
  ...
  plot_check_calibnodes( calibs = calib_nodes[[ i ]], divt_list = mcmc_priors[[ i ]], 
                         dat = dat[i], out = names( mcmc_priors )[count],
